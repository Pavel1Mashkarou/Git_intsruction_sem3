...
С помощью команды _git commit_ пользователь добавляет к последнему изменению файла через команду _git add_ комментарий. В зависимости от того, как мы прописываем данную команду, мы получим разные отклики терминала. Например:
...
- _git commit -am_
  ...
  Данная консольная команда не очень сильно отличается от _git commit -m_. Основное различие заключается в том, что _-am_ присваивает комментарий ко всем файлам.
...
- _git log --all_
  ...
  В случае, если пользователь с помощью команды _git checkout_ переместился в какое-либо сохранение, то резервные точки, которые были сохранены позже не будут отображаться. При использовании _git log --all_ будут показываться все изменения за всё время, независимо от времени сохранения.
